...
class TestDelayedQuery(TestCase):
    def setUp(self):
        ...
        # be appended.
        self.has_no_owner = Query.Or([Query.term_query("has_owner", False)])
        ...
        return (
            ...
            Query.And(
                [
                    Query.Or(
                        [
                            Query.term_query(f"{field}_id", "42"),
                            Query.term_query(f"{field}_id", "43"),
                        ],
                    ),
                    self.has_no_owner,
                ],
    ...
    #         Query.And(
    #             [
    #                 Query.Not(Query.term_query(f"{field}_id", "42")),
    #                 Query.Not(Query.term_query(f"{field}_id", "43")),
    #                 self.has_no_owner,
    #             ],
    #         ),
    #     )
    ...
    def test_get_permission_criteria(self):
        ...
        tests = (
            (None, [Query.term_query("has_owner", False)]),
            (User(42, username="foo", is_superuser=True), []),
            (
                User(42, username="foo", is_superuser=False),
                [
                    Query.term_query("has_owner", False),
                    Query.term_query("owner_id", 42),
                    Query.term_query("viewer_id", "42"),
                ],
            ),
        )
